...
async function tryOpenAI(topic: string, textInput: string, count: number) {
  ...
  const completion = await openai.chat.completions.create({ model: "gpt-4o-mini", messages: [{ role: "user", content: prompt }], temperature: 0.7, max_tokens: 2000 });
...
async function tryGemini(topic: string, textInput: string, count: number) {
  ...
  const promptText = `তুমি একজন BCS MCQ নির্মাতা। "${topic}" টপিকের উপর ${count}টি বাংলা MCQ JSON ফরম্যাটে দাও। ফরম্যাট: [{"question":"...","options":{"A":"...","B":"...","C":"...","D":"..."},"correctAnswer":"B","explanation":"...","difficulty":"easy","subtopic":"..."}]। সোর্স: ${textInput}`;
...
async function tryDeepSeek(topic: string, textInput: string, count: number) {
  ...
  const res = await fetch("https://api.deepseek.com/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: "deepseek-chat", messages: [{ role: "user", content: `তুমি একজন BCS MCQ নির্মাতা। "${topic}" টপিকের উপর ${count}টি বাংলা MCQ JSON ফরম্যাটে দাও। ফরম্যাট: [{"question":"...","options":{"A":"...","B":"...","C":"...","D":"..."},"correctAnswer":"B","explanation":"...","difficulty":"easy","subtopic":"..."}]। সোর্স: ${textInput}` }], temperature: 0.7, max_tokens: 2000 }),
  });
...
function generateDummy(text: string, topic: string, count: number) {
  ...
  for (let i = 0; i < Math.min(count, sentences.length); i++) {
    ...
    mcqs.push({ question: `"${s.substring(0, 50)}..." — নিচের কোনটি সঠিক?`, options: { A: opts[0], B: opts[1], C: opts[2], D: opts[3] }, correctAnswer: "ABCD"[opts.indexOf(correct)], explanation: correct, difficulty: "easy", subtopic: topic });
  }
  return mcqs;
}
...
export async function POST(req: Request) {
  try {
    ...
    if (process.env.OPENAI_API_KEY) { try { mcqs = await tryOpenAI(topic, textInput, count); used = "openai"; } catch {} }
    if (mcqs.length === 0 && process.env.GEMINI_API_KEY) { try { mcqs = await tryGemini(topic, textInput, count); used = "gemini"; } catch {} }
    if (mcqs.length === 0 && process.env.DEEPSEEK_API_KEY) { try { mcqs = await tryDeepSeek(topic, textInput, count); used = "deepseek"; } catch {} }
    if (mcqs.length === 0) { mcqs = generateDummy(textInput, topic, count); used = "dummy"; }
